...
float match(const cv::Mat* image1,
	    const cv::Mat* image2,
	    const cv::Mat* mask)
{    
...
void drawContour(cv::Mat* pImage,
		 const vector<cv::Point>& rContour,
...
//cout << "drawContour::END" << endl;
} // end of function


// Convert polar coordinates into cartesian coordinates
cv::Point convertPolarToCartesian(const cv::Point& rCenter,
			        int rRadius,
			        float rTheta)
{
...
//        cost->adjustROI(cost->cols-1,0,1,cost->rows);
        cv::Point max_loc ;        
...
void processAnisotropicSmoothing(const cv::Mat* pSrc,
				 cv::Mat* pDst,
				 int iterations,
				 float lambda)
{
...
chrono::steady_clock::time_point end1 = std::chrono::steady_clock::now();
        ...
        // Smooth image
        processAnisotropicSmoothing(unwrapped,unwrapped,15,1);	
...
chrono::steady_clock::time_point end4 = std::chrono::steady_clock::now();
	...
	//cv::Mat viterbiLine(unwrapped->rows, unwrapped->cols, unwrapped->depth());
	//unwrapped->copyTo(viterbiLine);
...
chrono::steady_clock::time_point end5 = std::chrono::steady_clock::now();
	...
	//int x = 0;
	//for(auto& v : optimalPath)
	//{
	//   cv::line(viterbiLine, cv::Point(x,v), cv::Point(x,v), cv::Scalar(255,255,0));
	//    x++;
	//}
	//showImage(unwrapped, &viterbiLine);
        for ( int i = 0 ; i < optimalPath.size() ; i++ )
        {
            contour[i] = convertPolarToCartesian(rCenter,minRadius+optimalPath[i],rTheta[i]);
        }
chrono::steady_clock::time_point end6 = std::chrono::steady_clock::now();
/*
cout << "		findContour_unwrapRing = " << std::chrono::duration_cast<std::chrono::microseconds>(end1 - start).count() <<std::endl;
cout << "		findContour_Anisothropic = " << std::chrono::duration_cast<std::chrono::microseconds>(end2 - end1).count() <<std::endl;
cout << "		findContour_gradients = " << std::chrono::duration_cast<std::chrono::microseconds>(end3 - end2).count() <<std::endl;
cout << "		findContour_AdditionalMAsk = " << std::chrono::duration_cast<std::chrono::microseconds>(end4 - end3).count() <<std::endl;
cout << "		findContour_viteribi = " << std::chrono::duration_cast<std::chrono::microseconds>(end5 - end4).count() <<std::endl;
cout << "		findContour_convert = " << std::chrono::duration_cast<std::chrono::microseconds>(end6 - end5).count() <<std::endl;
 */ 
        // Release memory
	delete unwrapped;
        return contour ;
...
void reconstructMarkerByMask(const cv::Mat* pMarker,
			     const cv::Mat* pMask,
			     cv::Mat* pDst)
{
	cv::Mat* difference = new cv::Mat(pMask->rows, pMask->cols, pMask->depth());
	pMask->copyTo(*difference);
	cv::Mat* mask = new cv::Mat(pMask->rows, pMask->cols, pMask->depth());
	pMask->copyTo(*mask);
	pMarker->copyTo(*pDst);
	...
	while( cv::sum(*difference).val[0])
	{
		...
		cv::dilate(*pDst, *pDst, structuringElement);
...
{
//    cout << "fillWhiteHoles" << endl;
	int width, height;
	width = pSrc->cols;
	height = pSrc->rows;
	
	cv::Mat* mask = new cv::Mat(cv::Size(width+2,height+2), pSrc->depth(), cv::Scalar(0));
	pSrc->copyTo((*mask)(cv::Rect(1,1,width,height)));
	cv::Mat* marker = new cv::Mat(mask->rows, mask->cols, mask->depth(), cv::Scalar(0));
	mask->copyTo(*marker);
	marker->zeros(marker->rows, marker->cols, marker->type());
	
	cv::rectangle(*marker, cv::Point(1,1), cv::Point(width+1, height+1), cv::Scalar(255));
	
	cv::Mat* result = new cv::Mat(mask->rows, mask->cols, mask->depth(), cv::Scalar(0));
	mask->copyTo(*result);
	reconstructMarkerByMask(marker, mask, result);
	(*result)(cv::Rect(1,1,width,height)).copyTo(*pDst);
